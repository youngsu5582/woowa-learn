...
import static org.example.woowalearn.acceptance.step.then.CommonThenStep.메일_발송한지_검증;
...
class AdminAcceptanceTest {
    ...
    @Test
    void 어드민이_지원서를_승인한다() {
        final var 액세스_토큰 = 회원가입후_토큰_반환(이메일,비밀번호)
                .accessToken();
    ...
    void 어드민이_지원서를_거절한다() {
        ...
        조회_성공한지_검증(결과);
        final var 지원서_조회_결과 = 본인_지원서_조회(액세스_토큰);
        지원_결과_특정_상태인지_검증(지원서_조회_결과, FormStatus.DENY);
    }

    @Test
    void 지원서_결과에_상관없이_메일을_발송한다() {
        final var 액세스_토큰 = 회원가입후_토큰_반환(이메일,비밀번호)
                .accessToken();
        final var 교육자_지원서_정보 = 교육자_지원서_정보_성성(
                신청_이유, 전화번호, 참고_링크, 계약자명, 연락_이메일
        ...
        final var 어드민_토큰 = 어드민_토큰_반환();
        지원서_거절(어드민_토큰, new ApplyChangeRequest(
                본인_지원서_결과_조회(액세스_토큰).applyFormId(),
                "지원 자격 미달"
        ));

        메일_발송한지_검증(이메일, 계약자명, 메일_발송자,"DENY");
    }
}
